export const chefSpecialities = [
    {
        namse: "House Special Chicken",
        dine_In_price: 17,
        take_out_price: 18,
        description: "Tender chicken chunks fried to a crisp & sautéed w rich garlic sauce.",
        spice: true,
        gluten: false
    },
    {
        name: "Vegetable Home Style",
        dine_In_price: 15.5,
        take_out_price: 15,
        description: "Broccoli, snow peas, zucchini, cabbage, fried tofu, mushrooms & carrots sautéed with gourmet hot garlic sauce.",
        spice: true,
        gluten: false
    },
    {
        name: "Champagne Chicken",
        dine_In_price: 17,
        take_out_price: 18,
        description: "Sliced white meat chicken, snow peas, water chestnuts & mushrooms sautéed with Chinese rice wine sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Shrimp & Chicken w. Basil",
        dine_In_price: 19.50,
        take_out_price: 18,
        description: "Large shrimp, white meat chicken & fresh garlic sautéed with basil sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Mongolian Trio",
        dine_In_price: 19.50,
        take_out_price: 19,
        description: "Beef, shrimp, chicken, scallions and onions sautéed w. light brown sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Coral Shrimp",
        dine_In_price: 20.5,
        take_out_price: 19,
        description: "Lightly battered shrimp stir-fried with red pepper, snow peas, celery & shredded ginger in Chef's special ginger sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Golden Sesame Chicken",
        dine_In_price: 19.50,
        take_out_price: 17,
        description: "Triple delight of shrimp, chicken and beef sautéed with broccoli, baby corn and mushrooms in a rich brown sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Triple Delight",
        dine_In_price: 19.50,
        take_out_price: 18,
        description: "Shrimp, chicken and beef sautéed with broccoli, baby corn and mushrooms in a rich brown sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Happy Family",
        dine_In_price: 23.50,
        take_out_price: 25,
        description: "Fillet mignon, shrimp and chicken sautéed with assorted vegetables in a white wine sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "House Special Steak Kew",
        dine_In_price: 23.50,
        take_out_price: 25,
        description: "Fillet mignon sautéed with vegetables in gourmet brown sauce.",
        spice: null,
        gluten: false
    }, {
        name: "Perfect Match",
        dine_In_price: 18,
        take_out_price: 19,
        description: "Chicken and shrimp sautéed with peanut flavored brown sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "General Tso's Chicken",
        dine_In_price: 17,
        take_out_price: 17,
        description: "Delicate breaded chicken lightly fried to perfection.",
        spice: null,
        gluten: false
    },
    {
        name: "Shrimp and Steak Kew",
        dine_In_price: 23,
        take_out_price: 25,
        description: "Shrimp and Steak sautéed with snow peas & zucchini in Szechuan chili sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Chicken with Pine Nuts",
        dine_In_price: 17,
        take_out_price: 17,
        description: "Sautéed chicken and mixed veg. in brown sauce, topped with pine nuts.",
        spice: null,
        gluten: false
    },
    {
        name: "Hunan Fish Fillet",
        dine_In_price: 18,
        take_out_price: 19,
        description: "Lightly fried fillet of flounder, showered with rich Hunan chili sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Prawn and Phoenix",
        dine_In_price: 18,
        take_out_price: 19,
        description: "Chicken & shrimp sautéed w. snow peas & zucchini in Szechuan chili sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Chicken Country Style",
        dine_In_price: 17,
        take_out_price: 18,
        description: "Chicken strips sautéed w. onion, pepper & baby corn in Chinese satay sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Empress Chicken",
        dine_In_price: 17,
        take_out_price: 18,
        description: "Chicken w. broccoli, water chestnuts and mushroom in Hunan chili sauce.",
        spice: null,
        gluten: false
    },
    {
        name: "Tofu Casserole with Seafood",
        dine_In_price: 19,
        take_out_price: 20,
        description: "Fried tofu, shrimp, scallops, squid, black mushrooms, water chestnuts, cabbage and baby bok choy in a light brown sauce",
        spice: null,
        gluten: false
    }
]